...
The files are produced in the following way:
  ...
  - Then the same is don for the data in UCI HAR Dataset/train which has the same
    format.
  ...
  - Then another table is produced with the averages of the values contained in
    each of the columns and that is saved to UCI_HAR_Dataset_averages_tidy.txt
